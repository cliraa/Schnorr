...
fn verification() {
    ...
    let sG = gx_gy.mul(s_scalar).unwrap_syscall();
    
    let xx_yy = secp256k1_new_syscall(Xx,Xy).unwrap_syscall().unwrap();
    let eX = xx_yy.mul(e_scalar).unwrap_syscall();
    
    let rx_ry = secp256k1_new_syscall(Rx,Ry).unwrap_syscall().unwrap();
